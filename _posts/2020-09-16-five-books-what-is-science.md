...
观测天文学是一门随着设备的进步，新结果不断取代旧结果的「后浪」学科。_The Realm of the Nebulæ_ 中的很多定量结论后来再看都是不精确的（例如，汇报的哈勃常数 _H_<sub>0</sub> 比当今测量值大七倍），谈及的宇宙模型也很原始。但我认为，这本书的价值正在于它**对自身局限的探讨**——Hubble 开篇就强调了**科学定律的本质：观测数据「合理但不唯一[^plausible]」的外推** (extrapolation)。而人类从航天器飞行范围外的星空中获取信息的唯一常规方式就是以视界为限的被动观测，并且「我们的知识随距离的增长迅速衰减；在望远镜昏暗的极限边缘，我们能测的只有阴影，然后在幽灵般的错误中寻觅重要的路标[^neighbor]」。在这样的条件限制下，天文学的「上层理论」无不包含大量的外推——它们能被接受主要是因为「**逐渐积累的内在一致性**，而非关键的决定性实验[^superstructure]」。比如，星云光谱红移是事实，但当时并没有证据表明它一定来自星系退行的多普勒效应；所以，认为红移是宇宙膨胀的「实验基础」其实也夹杂着天文学家对观测数据的主观诠释 (interpretation)。在第五章的结尾，Hubble 这样总结自己提出的定律：
